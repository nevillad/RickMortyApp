...
protocol ListViewPresentationLogic {
    func presentListViewDetails(response: ListViewModel.ListViewDetails.Response)
    func presentNextScene(response: ListViewModel.NextScene.Response)
    func presentLoader(type: ListViewLoaderType)
    func hideLoader(type: ListViewLoaderType)
    func presentError(type: ListViewErrorType)
}

class ListViewPresenter: ListViewPresentationLogic {
    weak var viewController: ListViewDisplayLogic?

    // MARK: Do ListViewDetails

    func presentListViewDetails(response: ListViewModel.ListViewDetails.Response) {
        let viewModel = ListViewModel.ListViewDetails.ViewModel()
        viewController?.displayListViewDetails(viewModel: viewModel)
    }

    func presentNextScene(response: ListViewModel.NextScene.Response) {
        let viewModel = ListViewModel.NextScene.ViewModel()
